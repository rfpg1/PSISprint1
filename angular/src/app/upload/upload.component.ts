      ...
      if (!(this.name === undefined)) {
        if (this.descricao === undefined) {
          if (confirm("Quer enviar a imagem sem a descricao")){
            boas = { user: user, name: this.name, likes: 0, descricao: "", date: new Date(), photo: reader.result as String }
            this.imageService.addPhoto(boas).subscribe(r => { window.location.reload() })
          }
        } else {
          boas = { user: user, name: this.name, likes: 0, descricao: this.descricao, date: new Date(), photo: reader.result as String }
          this.imageService.addPhoto(boas).subscribe(r => { window.location.reload() })
        }
      } else {
        if (this.descricao == undefined) {
